/**
 * Fase 5B Playwright E2E (CommonJS).
 * node experiments/smoke_e2e_sentimentos/fase5b/run_playwright.cjs
 */
...
const FE = process.env.FASE5B_FE_URL || "http://localhost:5173";
...
async function main() {
  ...
  try {
    ...
    if (!email || !password) {
      ...
      // Login fica em Configurações (CloudAuthPanel)
      const cfg = page.getByRole("button", { name: /Configura/i }).first();
      ...
      // memberships/isGestor podem carregar após o login
      try {
        await page.getByRole("button", { name: /Administração|Minhas turmas/i }).first().waitFor({ timeout: 15000 });
      } catch (_) {}
      await page.waitForTimeout(1500);
      await shot(page, "03_after_login");
      report.sidebar = (await page.locator(".sidebar-item, nav button, aside button").allTextContents())
        .map((s) => s.trim())
        .filter(Boolean);
      const hasGestorNav = report.sidebar.some((s) => /Administração/i.test(s));
      push("gestor_login", report.sidebar.length > 0, {
        sidebar: report.sidebar.slice(0, 25),
        is_gestor_nav: hasGestorNav,
      });
    }
    ...
    const clickNav = async (label, shotName) => {
      ...
      if ((await btn.count()) === 0) {
        push(`nav_${shotName}`, false, { result: "NÃO OBSERVADO", error: `not found: ${label}` });
        return false;
      }
      await btn.click();
    ...
    // Admin: label depende do papel (Administração vs Minhas turmas)
    const adminOk = await clickNav("Administração|Minhas turmas", "04_admin");
    for (const [label, name] of [
      ["Minha escola", "05_escola"],
      ["Turmas", "06_turmas"],
      ["Alunos", "07_alunos"],
      ...
      if ((await b.count()) > 0) {
        ...
        push(`section_${name}`, false, {
          result: "NÃO OBSERVADO",
          note: adminOk ? "subnav gestor não visível (papel sem Administração completa)" : "admin não aberto",
        });
      }
    }
    // Produto atual usa Relatórios (não "Dashboard")
    await clickNav("Relat[oó]rios", "09_dashboard");
    await clickNav("Escola", "05b_escola_produto");
    await clickNav("Ao vivo", "10_ao_vivo");
    await clickNav("Hist[oó]rico", "11_historico");
    await clickNav("Configura", "12_config");

    for (const label of ["Relat", "Ao vivo", "Hist", "Escola"]) {
      const b = page.getByRole("button", { name: new RegExp(label, "i") }).first();
    ...
    // Contar NÃO OBSERVADO separado de FAIL no summary helper
    report.nao_observado = report.cases.filter((c) => c.result === "NÃO OBSERVADO").length;
  } catch (e) {
  ...
  report.summary = {
    ...
    fail: report.cases.filter((c) => !c.ok && c.result !== "NÃO OBSERVADO").length,
    nao_observado: report.cases.filter((c) => c.result === "NÃO OBSERVADO").length,
  };
  writeFileSync(join(JSON_DIR, "fase5b_playwright.json"), JSON.stringify(report, null, 2), "utf8");
  console.log(JSON.stringify({ summary: report.summary, cases: report.cases.map((c) => [c.name, c.result]) }, null, 2));
